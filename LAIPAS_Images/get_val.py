...
def sample_from_patient_labeled(patient_id, stratify_set, root = '/data/LAIPAS_Images/'):
    ...
    if resize:
        ...
        msk = msk.resize((w,h), Image.Resampling.NEAREST)
    ...
    return (transforms.ToTensor()(img), torch.tensor(np.array(msk,dtype='uint8')))

def get_val(setname='val_stratify.npy', batch_size=16, n_workers=16):
    batch_size=16
    n_workers=16

    my_set = np.load('train_stratify.npy',allow_pickle=True).item()
